...
def clean_len(text: str) -> int:
    """
    Вычисляет видимую длину строки, игнорируя ANSI-коды управления цветом.

    Поскольку управляющие последовательности (например, \033[95m) занимают
    место в строке, но не отображаются в консоли, стандартная функция len()
    возвращает некорректное значение для выравнивания. Данная функция
    удаляет эти коды перед подсчетом.

    Args:
        text (str): Строка, содержащая текст и возможные ANSI-коды цвета.

    Returns:
        int: Реальное количество символов, которые будут отображены на экране.
    """
...
def enter_continue() -> None:
    ...
    Используется для фиксации важных игровых сообщений на экране, чтобы игрок
    успел их прочитать. После нажатия ENTER функция очищает консоль, подготавливая
    её для следующего кадра интерфейса.

    Returns:
        None: Функция блокирует выполнение потока до получения ввода от пользователя.
    """
    showing = True
    while showing:
        print()
...
def clear_display() -> None:
    """
    Выполняет полную очистку консольного окна.

    Использует системную команду 'cls' (для ОС Windows) для удаления
    всего предыдущего текста из терминала. Применяется для обновления
    кадров интерфейса и предотвращения мерцания при перерисовке.

    Returns:
        None: Функция отправляет команду напрямую в системную оболочку.
    """
    os.system('cls')
...
def loot_message(loot: int) -> None:
    """
    Выводит в консоль уведомление о результатах поиска предметов.

    На основе идентификатора полученного лута формирует сообщение для игрока.
    Информирует либо об успешном получении конкретного предмета (например, ремкомплекта),
    либо об отсутствии полезных находок в исследованном объекте.
...
def waiting_animation(duration: float = 6.0) -> None:
    """
    Отображает в терминале цикличную анимацию ожидания (spinner).

    Анимация использует символы шрифта Брайля и перезаписывает один и тот же
    символ в консоли, создавая эффект вращения.

    Args:
        duration (float): Продолжительность анимации в секундах. По умолчанию 6.0.

    Returns:
        None: Функция выполняет вывод в stdout и не возвращает значений.
    """
    frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
